...
class Promotion extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('from_date, to_date, roomtypes, type, min_stay', 'required', 'message' => 'Please enter {attribute}'),
			array('breakfast, accept_amend_cancel, hotel_id, no_of_day, updated_by, status, min_stay, max_stay, min_room, increase', 'numerical', 'integerOnly'=>true),
			array('discount, every_night, pickup, dropoff', 'numerical'),
			array('roomtypes, discount_type', 'length', 'max'=>50),
			array('promotion_code', 'length', 'max' => 8),
			array('type, package_type, blackout_from, blackout_to', 'length', 'max'=>100),
			array('book_on, cover_photo, apply_on', 'length', 'max'=>128),
	...
	public function search()
	{
		...
		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
			'sort' => array('defaultOrder' => 'status asc')
		));
	}
