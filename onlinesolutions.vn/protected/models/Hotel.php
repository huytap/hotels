...
class Hotel extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('chain_id, hotel_id', 'required', 'message' => 'Please enter {attribute}', 'on' => 'create'),
			...
			array('hotel_id, chain_id', 'length', 'max'=>128),
			array('city, website, term_condition, country, promotion, address, location, description, lat, lng, short_description, special_offer, trip', 'safe'),
			array('cover_photo, tel', 'length', 'max'=>128),
			array('private_local_css,private_link_css', 'length', 'max'=>256),
	...
	public function getHotelByHotelID($HotelID, $ChainID){
		$criteria = new CDbCriteria;
		$criteria->addCondition('hotel_id="'.$HotelID.'" AND chain_id="'.$ChainID.'" AND status=0');
		$hotel = Hotel::model()->find($criteria);
		return $hotel;
	}
	...
	public function getListByChain($chain_id){
		$criteria = new CDbCriteria;
		$criteria->compare('status', 0, false);
		$criteria->compare('chain_id', $chain_id, false);
		$dataProvider = new CActiveDataProvider($this, array(
			'criteria' => $criteria,
			'sort'=>array('defaultOrder'=>'display_order asc')));
		$dataProvider->setPagination(false);
	...
	public function getList4($arr_hotel){
		$criteria = new CDbCriteria;
		$criteria->compare('status', 0, false);
		if(count($arr_hotel) > 0){
			$i=0;
			$condition = '';
			foreach($arr_hotel as $ct){
				$condition .= 'FIND_IN_SET('.$ct.', `hotel_id`)';
				if($i < count($arr_hotel)-1){
					$condition .=' OR ';
				}
				$i++;
			}
		}
		//$criteria->compare('category_id',$_REQUEST['filter']['category'], false);
	...
	public function getHotelBySlug2($slug, $chain_id){
		$criteria = new CDbCriteria;
		$criteria->compare('slug', $slug, false, 'AND');
		$criteria->compare('status', 0, false, 'AND');
		$criteria->compare('chain_id', $chain_id, false);
	...
	public static function getByHotelID_ChainID($hotel_id, $chain_id){
		$criteria = new CDbCriteria;
	...
	public static function getByChainID($chain_id){
		$criteria = new CDbCriteria;
    	$criteria->compare('chain_id', $chain_id, false);
    	$criteria->compare('status', 0, false);
    	$data = Hotel::model()->findAll($criteria);
    	return $data;
	}
}
