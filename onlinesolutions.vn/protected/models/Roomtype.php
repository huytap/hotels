...
class Roomtype extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('hotel_id, display_order, no_of_rooms, max_per_room, status, updated_by, no_of_adult, no_of_extrabed, no_of_child', 'numerical', 'integerOnly'=>true),
			array('name, slug, view', 'length', 'max'=>100),
			array('bed', 'length', 'max'=>765),
			array('amenities', 'safe'),
			array('from_date, to_date, size_of_room, price', 'length', 'max'=>50),
	...
	public function getList3($hotel){
		$criteria = new CDbCriteria;
    ...
    public function getPrice($hotel, &$from, &$to, $id){	        
        ...
        $from = $rates['date']?$rates['date']:$from;
    ...
    public function getListToGallery($status='', $hotel, $arr_roomtype_id=''){
		$criteria = new CDbCriteria;
		if($status !== '')
			$criteria->compare('status', $status, false, 'AND');
		$criteria->compare('hotel_id', $hotel, false, 'AND');
		if($arr_roomtype_id){
			$criteria->addCondition('id not in('.$arr_roomtype_id.')');
		}
		$dataProvider = new CActiveDataProvider($this, array('criteria' => $criteria));
		$dataProvider->setPagination(false);
		$arrTheList = array();
	...
	public function getListOther($roomtype_id, $hotel_id){
    	$criteria = new CDbCriteria;
		$criteria->condition='t.id!='.$roomtype_id;
