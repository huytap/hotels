...
class ExtraHelper{
    ...
    public static function _checkAuth(&$hotel_id){
        $auth = getallheaders();

        if(!isset($auth['apikey'])) {
            self::_sendResponse(401);
        }
        $lang_config = Yii::app()->params['language_configs'];
        if(isset($auth['lang']) && isset($language_configs[$auth['lang']])){
            Yii::app()->language = $auth['lang'];
        }
        $criteria = new CDbCriteria;
        $criteria->compare('api_key', $auth['apikey'], false);
        $criteria->compare('chain_id', $auth['chain_id'], false);
        $check = Chain::model()->find($criteria);
        if($check === null) {
            self::_sendResponse(401, 'Error: User Name is invalid');
        }
        $hotel = Hotel::model()->getHotelByHotelID($auth['hotel_id'], $auth['chain_id']);
        $hotel_id = $hotel['id'];
    }
    ...
    public static function roundMoney($currency, $money){
        $exchangeRate = ExchangeRate::model()->exchangeRate2('USD', 'VND');
