...
class Bookings extends CActiveRecord
{
	...
	public function rules() 
    {
        return array( 
            ...
            array('book_from, country, srcFile', 'length', 'max'=>100),
            array('currency', 'length', 'max'=>10),
            array('notes, step', 'length', 'max'=>765),
            array('token, seats, verison, code, updated_date', 'safe'),
	...
	public function getByEmail_ShortID($email, $short_id){
		$criteria = new CDbCriteria;
		$criteria->compare('email', strtolower($email), false, 'and');
		$criteria->compare('short_id', strtolower($short_id), false, 'and');
		$criteria->addCondition('(status="confirmed" OR status="amended") AND checkin>"'.date('Y-m-d').'"');
		$booking = Bookings::model()->find($criteria);
		return $booking;
	}

	public function sentEmailCVV(){
		$now = date('Y-m-d');
        $checkin = date('Y-m-d', strtotime("$now + 9 days"));
        $criteria = new CDbCriteria;
        $criteria->addCondition('DATE(request_date)>="'.$now.'" AND checkin<="'.$checkin.'" AND status in("amended", "confirmed")');
        //$criteria->addInCondition('status', array('amended', 'comfirmed'));
        $criteria->compare('sent_cvv', 0, false);
        $criteria->compare('confirm_cvv', 0, false);
	...
	public function autoCheckout(){
        ...
        $criteria->addCondition("checkout<='".$yesterday."'");
        $criteria->compare('is_checkout', 0, false);
        
        $booking = new CActiveDataProvider($this, array(
        	'criteria' => $criteria,
        	'pagination' => array('pageSize'=>1),
        	'sort' => array('defaultOrder' => 'request_date desc')));
        //$booking->setPagination(false);
        return $booking;
	}
}
