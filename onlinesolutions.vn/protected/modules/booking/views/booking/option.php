...
<script type="text/javascript" charset="utf-8">
    var pickup_date = "<?php echo $booked['checkin'] ?>";
    var dropoff_date = "<?php echo $booked['checkout'] ?>";
</script>
            ...
            <?php
                    ...
                    <div class="row">
                        ...
                        <?php
                            ...
                            foreach($pickups as $p => $pickup){
                                $arrThePickup[$p] = str_replace('_',' ',$p) .' - ' .ExtraHelper::roundMoney3('VND',$pickup * $booked['exchangeRate']) .' ' .$booked['currency'];
                            }
                            echo $form->dropDownlist($booking, 'pickup', array('' => '-------------')+$arrThePickup,array('id'=>'pickup', 'options' => array($selected => array('selected' => true))))?>
                        <?php }?>
                        </div>
                    ...
                    <div class="row">                        
                        ...
                        <?php
                            ...
                            foreach($dropoffs as $d => $dropoff){
                                $arrTheDropoff[$d] = str_replace('_',' ',$d) .' - ' .ExtraHelper::roundMoney3('VND',$dropoff * $booked['exchangeRate']) .' ' .$booked['currency'];
                            }?>
                            <?php echo $form->dropDownlist($booking, 'dropoff', array('' => '-------------')+$arrTheDropoff,array('id'=>'dropoff'))?>
                        <?php }?>
                        </div>
                        <div class="col-sm-9 no-padding" id="drop_info" style="<?php if((isset($booking['dropoff']) && $booking['dropoff'] !== NULL)  || $promotion->dropoff) echo 'display:block;'; else echo 'display:none;';?>">
                            <div class="col-sm-4">
                ...
                if($room['no_of_extrabed']>0){
                    ...
                    for($f=1;$f<=$booked['no_of_room'];$f++){
                        ...
                        echo '<div class="row"><div class="col-sm-12"><strong>'.Yii::t('lang', 'extrabed_room').' '.$f.'</strong></div></div>';
                            ...
                            <div class="row extra">
                                <div class="col-sm-12"><input <?php echo $checked;?> type="checkbox" id="extrabed<?php echo $et?>" class="input-checkbox" value="<?php echo $pk['id']?>" <?php echo $checked;?>> <?php echo ucfirst(Yii::t('lang', 'extrabed')).' ' .$booked['currency'] .' '.ExtraHelper::roundMoney3('VND',Settings::model()->getSetting('extrabed',$booked['hotel_id'])*$booked['exchangeRate']);?></div>
                            </div>
                    <?php
                    $et++; 
                    }
                ...
                if(count($package->getData())>0){
                    ...
                    foreach($package->getData() as $pk){
                        ...
                        $price2 = $booked['currency'].' '.ExtraHelper::roundMoney3('VND',$pk['rate']*$booked['exchangeRate']) .' per adult '.$booked['currency'].' '.ExtraHelper::roundMoney3('VND',$pk['rate_child']*$booked['exchangeRate'], 2).' per child per night';
                        if(in_array($pk['id'], $pr)){
                            ...
                            $checked='checked="checked"';
                            $display = '';
                            $price = $booked['currency'].' '.ExtraHelper::roundMoney3('VND',($booked['packages'][$pk['id']]['price_adult']+$booked['packages'][$pk['id']]['price_child']), 2);
                        }?>
        ...
        <div class="col-md-3 col-sm-4 col-xs-12">
            ...
            <?php
                ...
                <div class="panel-body white">
                    ...
                    <hr>
                    <?php
                    $room_charge = $total_per_night*$booked['no_of_room']*$booked['nights'];
                    ?>
                    <div class="row">
                        <div class="col-md-12">
                            <strong><?=Yii::t('lang', 'room_charge')?></strong>
                            <span class="pull-right"><?=$booked['currency']?> <?=ExtraHelper::roundMoney3('VND',$room_charge);?></span>
                        </div>
                    </div>
                    <?php
                    $extras=0;
