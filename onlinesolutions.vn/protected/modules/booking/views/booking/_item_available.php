...
foreach ($room['promos'] as $promo){
    ...
    if ($promo['promotion_type'] == 'early_bird') {
        if ($promo['today'] < $promo['no_of_day'] && 
        	...
        	<div class="row promotions row-promos <?php echo $notshow?>" rel="<?php echo $promo['promotion_id'] ?>">
		  		...
		  		<div class="col-md-2 col-sm-3 col-xs-12 col rate-result center"><span class="right btn-disabled"><?php echo str_replace(0, $promo['no_of_day'], Yii::t('lang', 'require_checkin')); ?></span></div>
		  	</div>
        <?php
    ...
    elseif ($promo['promotion_type'] == 'last_minutes') {
        if ($promo['today'] > $promo['no_of_day']) {$flag=0;?>					
			<div class="row promotions row-promos <?php echo $notshow?>" rel="<?php echo $promo['promotion_id'] ?>">
		  		<?php $this->renderPartial('_promotion_item', compact(array('promo', 'p_des', 'room', 'lang', 'params', 'flag')));?>
		  		<div class="col-md-2 col-sm-3 col-xs-12 col rate-result center"><span class="right btn-disabled"><?php echo str_replace(0, $promo['no_of_day'], Yii::t('lang', 'last_1')); ?></span></div>
		  	</div>
            <?php
        } else {$flag=1;?>
			<div class="row promotions row-promos" rel="<?php echo $promo['promotion_id'] ?>">
        ...
        if ($promo['min_stay'] > 1) {
            if ($promo['bookedNights'] < $promo['min_stay'] || $promo['bookedNights'] < $promo['min_stay']) {
                ...
                <div class="row promotions row-promos <?php echo $notshow?>" rel="<?php echo $promo['promotion_id'] ?>">
			  		<?php $this->renderPartial('_promotion_item', compact(array('promo', 'p_des', 'room', 'lang', 'params', 'flag')));?>
			  		<div class="col-md-2 col-sm-3 col-xs-12 col rate-result center"><span class="right btn-disabled"><?php echo str_replace(0, $promo['min_stay'], Yii::t('lang', 'last_0')); ?></span></div>
			  	</div>
			...
			<?php
				...
				<div class="row promotions row-promos <?php echo $notshow?>" rel="<?php echo $promo['promotion_id'] ?>">
			  		<?php $this->renderPartial('_promotion_item', compact(array('promo', 'p_des', 'room', 'lang', 'params', 'flag')));?>
			  		<div class="col-md-2 col-sm-3 col-xs-12 col rate-result center"><?php echo str_replace(0, $promo['min_stay'], Yii::t('lang', 'last_1')); ?></div>
			  	</div>
                <?php
            }
        } else {
