        ...
        //try{
            ...
            if(isset($hotel_id) && isset($chain_id)){
                ...
                if($hotel){
                    ...
                    if(isset($_GET['children_age']) && $_GET['children_age']>0){
                        $params['children_age'] = $_GET['children_age'];
                    }
                    if(isset($_GET['children_age1']) && $_GET['children_age1']>0){
                        $params['children_age1'] = $_GET['children_age1'];
                    }

                    if(isset($_POST['children_age']) && $_POST['children_age']>0){
                        $params['children_age'] = $_POST['children_age'];
                    }
                    if(isset($_POST['children_age1']) && $_POST['children_age1']>0){
                        $params['children_age1'] = $_POST['children_age1'];
                    }
                    ...
                    if(isset($_POST['FormBook'])){
                        if(strtotime($_POST['FormBook']['checkin'])>=strtotime($now) && 
                            ...
                            $_POST['FormBook']['children']>=0){
                                ...
                                header('Location: '. Yii::app()->params['link'].'booking/'.$hotel_id.'/'.$chain_id.'/?checkindate='.$fromDate.'&checkoutdate='.$toDate.'&adult='.$_POST['FormBook']['adult'].'&children='.$_POST['FormBook']['children'].$currency.'&children_age='.$params['children_age'].'&children_age1='.$params['children_age1']);
                            }
                    }
                    $now = date('d-m-Y');
                    ...
                    $params['extrabed'] = Settings::model()->getSetting('extrabed',$hotel['id']);
                    ...
                    if($params['bookedNights']>0){
                        ...
                        /*book roomtype*/
                        $package = PackageHelper::getRoomRate($params);
    ...
    public function actionPrebook(){
        ...
        if(Yii::app()->session['_params'] && 
            ...
            if(!$flag){
                ...
                if($getRoomtype['no_of_rooms']>=$adult && isset($available[$getRoomtype['display_order']])){
                    ...
                    $price = $available[$getRoomtype['display_order']]['promos']['promos_'.$_POST['promotion']][$adult];
                    if($adult>2){
                        $price = $available[$getRoomtype['display_order']]['promos']['promos_'.$_POST['promotion']][2] + $available[$getRoomtype['display_order']]['extrabed_price']*($adult-2);
                    }
                    //echo "<pre>";print_r($available[$getRoomtype['display_order']]);
                    $cart['rate'] = $price;
                    ...
                    $price = $available[$getRoomtype['display_order']][$adult];
                    if($adult>2){
                        $price = $available[$getRoomtype['display_order']][2] + $available[$getRoomtype['display_order']]['extrabed_price']*($adult-2);
                    }
                    $cart['rate'] = $price;
                    $cart['no_of_room'] = $params['no_of_room'];
                    $cart['exchangeRate'] = $exchangeRate;
