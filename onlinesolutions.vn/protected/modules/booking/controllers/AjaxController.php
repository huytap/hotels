...
class AjaxController extends Controller{
    ...
    public function actionAutosendcvv(){
        try{
            $this->layout=false;
            $model = Bookings::model()->sentEmailCVV();
            if($model && count($model->getData()) >0){
                foreach($model->getData() as $booking){
                    //sent email alert cvv
                    $chain = Chain::model()->findByPk($booking['hotel']['chain_id']);
                    $now_checkin = date('d-m-Y');
                    $to_checkin = date('d-m-Y', strtotime($booking->checkin));
                    ...
                    if(strtotime($checkin_plug) <= strtotime($now_checkin)){
                        ...
                        $subject = $booking['hotel']['name']. ' – Credit Card Information Required for #'. strtoupper($book->short_id);
                        $data_alert = array(
                            'logo' => $booking['hotel']['logo1'],
                            'hotel' => $booking['hotel']['name'],
                            'hotelname' => $booking['hotel']['name'],
                            'chainname' => $full_name,
                            'fullname' => $booking['title'] .'. '.$booking['first_name'].' ' .$booking['last_name'],
                            'cardnumber' => $booking['four'],
                            'bookingid' => strtoupper($booking['short_id']),
                            'checkin' => $to_checkin
                        );
                        //$arr_cc = array('nghuytap@gmail.com' => 'Tap Nguyen');
                        //$email_reply = 'huytaptit@gmail.com';
                        $arr_cc = array($booking['hotel']['email_sales'] => 'Sales');
                        $email_reply = $booking['hotel']['email_sales'];
    ...
    public function actionPr_condition(){
        var_dump('expression');die;
        if(isset($_GET['pr'])){
            $pr = Promotion::model()->findByPk($_GET['pr']);
            $this->render('promotion', compact('pr'));
        }
    }
