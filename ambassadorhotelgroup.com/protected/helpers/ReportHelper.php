...
class ReportHelper{
	...
	public static function dataBooking($filter_data) {
		...
		if(is_array($arrTheList)){
			foreach($arrTheList as $k => $v){
	            ...
	            $tmp[] = $pr['en'];
	            $tmp[] = number_format($v->pickup_price, 2);
	            $tmp[] = number_format($v->dropoff_price, 2);
	            $tmp[] = number_format($v->total, 2);
	            $tmp[] = $v->status;
                $arr[] = $tmp;
			}
		}
	...
	public static function doExport($arrReport, $data, $arrCells=array(), $formated = FALSE, $array_bold = FALSE) {
      ...
      $objPHPExcel = $objReader->load("templates/" . $arrReport['template']);
