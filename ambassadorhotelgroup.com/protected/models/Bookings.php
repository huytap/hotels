...
class Bookings extends CActiveRecord
{
	...
	public function rules() 
    {
        return array( 
            ...
            array('card_number, sent_mail, card_cvv, view, roomtype_id, promotion_id, hotel_id, no_of_adults, no_of_child, no_of_extrabed, no_of_room', 'numerical', 'integerOnly'=>true),
            ...
            array('notes', 'length', 'max'=>765),
            array('token, seats, verison', 'safe'),
	...
	public function search($params)
	{
        ...
        if(isset($_REQUEST['filter']['fromDate']) && isset($_REQUEST['filter']['toDate'])){
        	$fromDate = ExtraHelper::date_2_save($_REQUEST['filter']['fromDate']);
			$toDate = ExtraHelper::date_2_save($_REQUEST['filter']['toDate']);
        	$criteria->addCondition('checkin >="'. $fromDate['date'] .'" AND checkout<="'. $toDate['date'] . '"');
        }
        
        if(isset($_REQUEST['filter']['request_date_from']) && isset($_REQUEST['filter']['request_date_to'])){
        	$request_fromDate = ExtraHelper::date_2_save($_REQUEST['filter']['request_from_date']);
			$request_toDate = ExtraHelper::date_2_save($_REQUEST['filter']['request_date_to']);
        	$criteria->addCondition('DATE(request_date) BETWEEN "'. $request_fromDate['date'] .'" AND "'. $request_toDate['date'] . '"');
	    }
        
        if(isset($_REQUEST['filter']['hotel'])){
        	$criteria->addCondition('hotel_id='.$_REQUEST['filter']['hotel']);
        }else{
